...
This is not an aesthetic choice; it was forced by a real constraint on the training side. The GPU training backend in use (Intel ITEX/oneDNN) refuses to run a Conv2D layer on single-channel input, so the network needed 3 input channels no matter what. Since a convolution's parameter count depends on the number of channels, not on what is inside them, there was no cost to putting real signal in that third channel instead of a lazily duplicated copy of the luminance channel. The model ends up seeing brightness plus two real color channels, which turned out to carry enough information for the CNN described next to reach the accuracy reported in §6.4, without ever needing the fragile cascade step.

### 6.2 Model architecture

The model is a custom convolutional neural network with 341,121 parameters, comfortably under a self-imposed 2-million-parameter budget, and it is trained from scratch with no transfer learning from a larger pretrained network. This is a deliberate choice: transfer learning from a large pretrained backbone such as MobileNetV2 was considered and set aside. The reasoning is that a system meant to eventually run on constrained embedded hardware should not lean on a large general-purpose vision backbone if a small, purpose-built network can do the job; a smaller model is faster to run, smaller to ship, and easier to reason about end to end. Four Conv2D+BatchNorm+MaxPooling blocks feed a `GlobalAveragePooling2D` layer instead of a large flatten-and-dense head, which keeps the parameter count independent of the input image's resolution: only the convolutions' compute cost scales with input size, not the model's weight count.

One finding along the way was genuinely counter-intuitive. An early training set had a 1.3:1 imbalance between the two classes, and the standard fix for that, passing `class_weight='balanced'` to the training loop, actually made results worse rather than better: the recall gap between classes widened from 0.61/0.89 to 0.51/0.91. The likely explanation is an interaction with early stopping, which watches validation loss to decide when to stop training; a class-weighted loss is a noisier signal for that mechanism to watch, so it tends to stop training at a worse point than an unweighted loss would. The final configuration trains with `USE_CLASS_WEIGHT = False`.

### 6.3 Dataset composition: filtering, merging, and a leakage bug

The first trained model used the HuggingFace `driver-drowsiness-dataset` (n7i5x9) as-is: 18,492 training images, 2,311 validation, 2,313 test, with its own provided splits kept strictly separate and never re-merged. That produced a reported accuracy of 75.8%. Live testing later exposed two serious problems with this setup, neither visible from the offline numbers alone.

The first was uncovered by testing under different lighting than before, in the exact camera setup that had previously worked: the model's confidence collapsed to near-random. Visually inspecting the actual training images, not just their labels, showed why: the dataset mixes full driver-in-car photos with extreme macro single-eye crops under the same two class labels. The publisher's own description claims these were filtered out; in practice they were not. A dedicated script, `models/filter_dataset.py`, uses OpenCV's Haar frontal-face detector to keep only images with a detectable face, discarding the macro-eye subset. Deliberately built as a standalone script with no TensorFlow import at all, since `cv2.CascadeClassifier` is the same component that reliably segfaults when run in the same process as TensorFlow (§6.1). Applying it to n7i5x9 alone discarded 62.3% of the dataset, and the remaining ~7,000 training images were too few for the model to converge well (validation accuracy fell to 65%).

To recover volume without reintroducing the macro-eye problem, a second HuggingFace dataset, `akahana/Driver-Drowsiness-Dataset`, was added after confirming a sample of it passed the same face filter at a 92.5% rate. This brought the combined training set to roughly 37,700 images and pushed validation accuracy up to 91.7% by the third epoch, an improvement large enough to be suspicious rather than simply good news.

That suspicion turned out to be justified. Checking for data leakage with perceptual image hashing (an average hash compared by Hamming distance) found that 99.6% of the new dataset's validation samples had a near-duplicate already sitting in the training set: the akahana source contains near-identical burst frames from the same recording sessions, with no subject or session identifier available to split on correctly, and a naive random per-image split scattered these duplicates across both sides of the split. Checking the original n7i5x9 splits under the same test, which had never been touched or re-split by this project, found the same problem already present upstream: 86.7% leakage. Every accuracy figure produced in this project up to that point, including the original 75.8%, had been inflated by this from the very start, and the in-progress training run was stopped immediately rather than let finish on data already known to be invalid.

The fix, `models/dedupe_split.py`, hashes every kept image, clusters near-duplicates together (a coarse hash buckets images cheaply before a finer Hamming-distance check within each bucket, avoiding a full pairwise comparison over roughly 47,000 images), and assigns whole clusters, never a fraction of one, to train, validation, or test in an 80/10/10 ratio. The clustering itself was revealing on its own: 25,084 "drowsy" images collapsed into only 1,769 distinct clusters, an average of about fourteen near-duplicate frames per real, unique moment. Retraining on this leak-safe split produced a validation accuracy of 77.4%, well below the earlier leaky 91.7%, but the first number in this project's history that can actually be trusted. A further manual pass removing a small number of macro-eye images that the automated face filter had missed (false negatives on close eye and eyebrow texture) improved this slightly further, to the number reported in §6.4.

One quantified finding from this investigation fed directly into the final model. Measuring the ratio of detected face size to full frame size across a sample of each source shows they sit at very different visual scales: faces fill an average of 74% of the frame in akahana's images, against 11.7% in n7i5x9's. Checked directly, this scale difference does not correlate with the class label inside either source on its own (drowsy versus not-drowsy faces are framed almost identically within akahana, and within n7i5x9), so it is not a second hidden leakage bug. It does mean the two sources carry very different amounts of usable eye detail once resized to a fixed input size, which was the leading suspect for why live performance depended heavily on the driver's distance from the camera.

Two changes targeted this directly. First, the input resolution was raised from 96×96 to 160×160. This is close to free against the parameter budget, since the `GlobalAveragePooling2D` layer (§6.2) decouples the model's weight count from its input size entirely; only the compute cost per image goes up, not the model itself. Second, `zoom_range` in the training augmentation was raised from 0.1 to 0.35, deliberately synthesizing bigger apparent distance swings from the existing photos. This is an approximation, not a substitute for genuinely varied real photos: zooming out on an already-cropped source image pads or repeats its edge pixels rather than revealing real background, so it does not fully reproduce what a genuinely distant camera shot would look like. It is still a reasonable, cheap thing to try before concluding nothing more can be done.

### 6.4 Results

| | precision | recall | f1-score | support |
|---|---|---|---|---|
| not_drowsy | 0.755 | 0.743 | 0.749 | 2073 |
| drowsy | 0.786 | 0.797 | 0.791 | 2453 |
| **accuracy** | | | **0.772** | 4526 |
...
Validation accuracy is 77.2%, measured on the same leak-safe split described in §6.3, at 160×160 resolution with the wider zoom augmentation. This is the final model this project ships. It is marginally lower than the 78.9% measured at 96×96 immediately after the leakage fix, a difference small enough to plausibly be noise between two runs rather than the resolution or augmentation change actually hurting; what it is not is anywhere near the 75.8%/82.7%/91.7% figures produced at earlier, since-discredited stages of this project.

The two classes remain close to each other in both precision and recall (0.755/0.743 for not_drowsy, 0.786/0.797 for drowsy), the same balance goal discussed throughout this project: for a safety alarm, a model that catches both classes roughly equally well matters more than a higher number produced by being much better at one than the other. Still below the original 85% target, and still limited live by the distance/position sensitivity discussed in §8.2, but arrived at through a training and evaluation process this report can actually vouch for, which the earlier, higher-looking numbers never were.
...
### 8.1 Four real bugs, found only on real hardware

None of the bugs below showed up anywhere in the unit test suite. All four only became visible once the system actually ran end to end on physical hardware, over a real WiFi network, talking to a real broker, and each one is a different flavor of problem that a mocked test simply has no way to see.
...
4. **LED matrix went dark during a sustained drowsy episode, even though the Python side still correctly considered the driver drowsy.** `AlertNotifier.notify()` only published an MQTT message on the initial false-to-true transition, on the reasoning that republishing on every frame while the state had not changed would be pointless traffic. The Arduino, however, has no notion of "the alarm should still be active": `TelemetryHandler` simply times out and turns the alarm off a fixed 2 seconds after the last message it actually received. Any drowsy episode lasting longer than that timeout, or a fast enough sequence of detections flickering around the decision threshold, could leave the Arduino's physical alarm off while Python's own internal state still read as active, with no further message ever arriving to correct it, since nothing had "changed" from Python's point of view. The fix adds a keep-alive: `AlertNotifier` now republishes at least once per second while `drowsy_detected` stays true, comfortably inside the Arduino's timeout window, so the physical alarm never goes dark on its own during a genuine, ongoing detection.
...
**Known limitation, partially addressed, still open:** the model's live behavior depends on the driver's exact distance and position relative to the camera, working most reliably within a fairly narrow window. The scale mismatch between the two training sources described in §6.3 was the leading suspect, so two mitigations were tried: raising the input resolution to 160×160 and substantially widening the training-time zoom augmentation, both described in §6.3. Live testing after both changes still shows the same qualitative pattern: reliable at the right distance, degrading away from it, though narrowing this further is limited by an approximation these mitigations cannot fully close, since synthetic zoom on already-cropped source photos cannot recreate the real background and detail loss of a genuinely distant photograph. Adding automatic face-cropping at inference time to normalize scale directly was, and remains, deliberately not attempted, because it would need the same Haar cascade that reliably segfaults when run in the same process as TensorFlow (§6.1), and live inference runs in exactly that process. DrowsyGuard should be treated as requiring a fixed, close-range camera position, the way a real driver-monitoring system would be installed, rather than a general-purpose webcam at an arbitrary distance.
...
- **Model accuracy (77.2% final, target was >85%): reopened, investigated thoroughly, still not fully closed.** What began as a decision to accept 75.8% and move on was reopened after a live test surfaced a real bug, which in turn uncovered a data leakage problem affecting every accuracy figure in this project's history (§6.3). The leakage is fixed, and two further mitigations (higher input resolution, wider zoom augmentation) were tried against the live distance-sensitivity symptom (§8.2). The original >85% target is still not met, and the live-behavior limitation is improved but not eliminated. This is a boundary drawn by time rather than by having run out of ideas: two more concrete avenues (a single-source retrain, real face-cropping at inference with a non-conflicting detector) were identified and left for later.
...
DrowsyGuard, as delivered, is a complete, tested, secure driver-drowsiness detection pipeline: five single-responsibility components built test-first, a lightweight custom CNN, AES-256-CBC encrypted telemetry with both a demonstrated adversarial attack and a working defense against it, and a physical Arduino companion that surfaced four real hardware bugs no unit test could ever have caught on its own, the last of them, a physical alarm silently going dark mid-detection, found only after everything else in this report was already believed finished. A later, deliberate coverage-driven review of the test suite itself, taking `src/` from 91% to 98% line coverage, found two further real bugs on top of that, one of them reproduced as an actual crash rather than argued about in the abstract. A live test near the end of the project uncovered a data leakage bug affecting every accuracy figure produced up to that point, including ones already accepted and reported on; rather than leave that standing, the split was rebuilt from scratch with near-duplicate clustering, at the cost of a lower but honest number. What remains open at the end of this project is stated plainly: a below-target model accuracy and a live-behavior limitation whose cause is understood and partially mitigated but not eliminated, both accepted under real time constraints, and a small, explicitly scoped set of "will not fix" items.

...
Validation accuracy is 78.9%, AUC 0.896, measured on the leak-safe split described in §6.3. This is, deliberately, reported instead of the higher but invalid 75.8%/82.7%/91.7% figures produced at earlier stages of this project: those numbers looked better and were wrong. This one is lower and is the one this report stands behind.

The two classes remain close to each other in both precision and recall (0.756/0.735 for not_drowsy, 0.781/0.799 for drowsy), the same balance goal discussed for the earlier, since-superseded model: for a safety alarm, a model that catches both classes roughly equally well matters more than a higher number produced by being much better at one than the other. Below the original 85% target, and, unlike the earlier figures, arrived at through a training and evaluation process this report can actually vouch for.
...
**Known limitation, still open:** the model's live behavior depends heavily on the driver's exact distance and position relative to the camera, working reliably only within a fairly narrow window. This has been investigated in depth, not just observed and left alone (§6.3): the two merged training sources sit at very different visual scales, with faces filling an average of 74% of the frame in one source and 11.7% in the other, and that scale gap is the leading suspect, since a face resized to the model's fixed 96×96 input carries much less real eye detail when it started out small in the original photo. Adding automatic face-cropping at inference time to normalize this was deliberately not attempted, because it would need the same Haar cascade that reliably segfaults when run in the same process as TensorFlow (§6.1), and live inference runs in exactly that process. Two concrete next steps were identified but not yet tried within this project's time budget: training on a single visually-consistent source with stronger synthetic zoom augmentation to manufacture scale diversity, or raising the model's input resolution above 96×96, which costs almost nothing against the parameter budget thanks to global average pooling (§6.2) and would preserve more real detail from the smaller-scale source. Until one of these is tried, DrowsyGuard should be treated as requiring a fixed, close-range camera position, the way a real driver-monitoring system would be installed, rather than a general-purpose webcam at an arbitrary distance.
...
- **Model accuracy (78.9%, target was >85%): reopened, improved, still not fully closed.** What began as a decision to accept 75.8% and move on was reopened after a live test surfaced a real bug, which in turn uncovered a data leakage problem affecting every accuracy figure in this project's history (§6.3). The leakage is fixed and the current 78.9% is the first trustworthy number produced. The original >85% target is still not met, and a further live-behavior limitation (distance/position sensitivity, §8.2) remains open with concrete next steps identified but not yet attempted, a boundary drawn by time rather than by having run out of ideas.
...
DrowsyGuard, as delivered, is a complete, tested, secure driver-drowsiness detection pipeline: five single-responsibility components built test-first, a lightweight custom CNN, AES-256-CBC encrypted telemetry with both a demonstrated adversarial attack and a working defense against it, and a physical Arduino companion that surfaced three real hardware bugs no unit test could ever have caught on its own. A later, deliberate coverage-driven review of the test suite itself, taking `src/` from 91% to 98% line coverage, found two further real bugs on top of that, one of them reproduced as an actual crash rather than argued about in the abstract. A live test near the end of the project uncovered a data leakage bug affecting every accuracy figure produced up to that point, including ones already accepted and reported on; rather than leave that standing, the split was rebuilt from scratch with near-duplicate clustering, at the cost of a lower but honest number. What remains open at the end of this project is stated plainly: a below-target model accuracy and a live-behavior limitation whose cause is understood but not yet fixed, both accepted under real time constraints, and a small, explicitly scoped set of "will not fix" items.
